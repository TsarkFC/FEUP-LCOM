...
extern int counter;//declared in timer.c
...
int(timer_test_read_config)(uint8_t timer, enum timer_status_field field) {
  uint8_t flag; //flag will receive status byte
  //GET CONF returns 1 in case of faillure, interpreted as TRUE
  if (timer_get_conf(timer, &flag)) return 1;
  //DISPLAY CONF returns 1 in case of faillure, interpreted as TRUE
  if (timer_display_conf(timer, flag, field)) return 1;
  return 0;
}
